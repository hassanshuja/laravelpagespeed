var br = "/";
var offerType = 'alle', offerRegion = 'alle', offerDuration = 'alle', keyw = 'alle', vaucherCustom = 100, activateEnter = !1, jssor1_slider = null, validator = null, scrollHistory = [];
var activeOffer = 0;
var bookingDate = 0;
window["toggleDivWithclass"] = function (togglediv, thName) {
    if ($(window).width() < 768) {
...
window['activateDate'] = function () {
    if ($('[type="date"]').prop('type') != 'date') {
        $('[type="date"]').datepicker()
    }
}

window["toggleDivWithFilter"] = function (togglediv, thName) {
    if ($(window).width() < 992) {
        $('#' + togglediv).slideToggle('none', function () {
        }
        )
    }
...
window['showImage'] = function (image, vType) {
    $('#mainImage').attr('src', br + image);
    $('#mainImage').fadeOut();
    $('#mainImage').fadeIn();
    $('#voucherType').val(vType)
}

window['showImageVoucherMultiple'] = function () {
    $('#multiplePhoto').show();
    $('#onePhoto').hide();
    $('#vauchTemp').val(0)
}

window['showImageVoucherSingle'] = function (image, vType) {
    $('#multiplePhoto').hide();
    $('#onePhoto').show();
...
window["changeDetectorInput"] = function () {
    var today = new Date();
    ...
    $(document).on('keyup', "#exampleTextarea", function () {
        var txtVal = $("#exampleTextarea").val().replace(/\n/g, "<br />");
        $("#voucherDescArea").html(txtVal)
    }
    );
    $(document).on('keyup', "#offerVaucherTextarea", function (e) {
        var txtVal = $("#offerVaucherTextarea").val().replace(/\n/g, "<br />");
        e.currentTarget.innerHTML = $("#offerVaucherTextarea").val();
        $("#voucherDescArea").html("<p>" + txtVal + "<p>")
    }
    );
    $(document).on('keyup', "#vouchName", function () {
        var txtVal = this.value;
        $("#vouchNameTag").html(txtVal)
    }
    );
...
$(document).ready(function () {
    ...
    $(document).on('click', "#confirmContact", function (eve) {
        var captcha = grecaptcha.getResponse();
        cRules = {
            user_name: "required", user_surname: "required", user_address: "required", user_postalCode: "required", user_postalPlace: "required", user_country: "required", user_telephone: "required", user_email: "required", user_comment: "required", andreOp: "required"}
        ;
        cMessages = {
            user_name: {
        ...
        $("#contactForm").validate({
            errorClass: "bad-message", rules: cRules, messages: cMessages, submitHandler: function (form) {
                ...
                sendData = {
                    _token: token, andrede: uStatus, name: uName, vorname: uSurname, strasse: uAddress, ort: uPostalCode, plz: uPostalPlace, telefon: uTelephone, email: uEmail, company: uCompany, message: uComment, offer: uOffer, }
    ...
    $(document).on('click', "#sendGroupOffer", function (eve) {
        cRules = {
            user_postalCode: "required", user_postalPlace: "required", user_telephone: "required", user_email: "required", }
        ;
        cMessages = {
            user_postalCode: {
                required: "Bitte um Ihre Angaben: PLZ"}
        ...
        $("#groupOfferForm").validate({
            errorClass: "bad-message", rules: cRules, messages: cMessages, submitHandler: function (form) {
                ...
                var token = $("input[name=_token]").val();
                sendData = {
                    _token: token, offer: excursionPack, date: gDate, lastName: uSurname, firstName: uName, company: uCompany, street: uAddress, plz: uPostalPlace, ort: uPostalCode, telefon: uTelephone, email: uEmail, alternative: alternativT, starttime: desiredStTime, remarks: uRemarks, total_people: noParticipants}
    ...
    $(document).on('click', "#voucherStepb1,#voucherStepb2,#voucherStepb2Pay,#voucherStepb3", function (eve) {
        ...
        if (vStepId == 'voucherStepb1')
        {
            ...
            var vText = $('#exampleTextarea').val().replace(/\n/g, "<br />");
            var vPrice = $('#otherValue').val();
            var optionsValues = $('input[type=radio][name=optionsRadios]:checked');
            $("#voucherFor").text(vFor);
            $("#voucherFor1").text(vFor);
            $("#voucherText").html(vText);
            $("#voucherText1").html(vText);
            if (vaucherCustom == "other")
            {
        ...
        } else if (vStepId == 'voucherStepb2')
        {
            ...
            var optionsValues = $('input[type=radio][name=optionsRadios]:checked');
            liberInd = uStatus;
            console.log(vaucherCustom,'vaucherCustom',vPrice,optionsValues.val());
            if (optionsValues.val() == "other")
            {
                dbPayVal = vPrice
            } else {
            ...
            var token = $("input[name=_token]").val();
            sendData = {
                _token: token, vaucher_type: 2, payment_id: dbPay, offer: 0, valid_from: uValidFrom, vaucher_for: vFor, vaucher_text: vText, items: '', payment_type: uPayment, send_type: uShipping, message: uMessage, vaucher_template: voucherType, total_price: dbPayVal, user: {
...
$( document ).on('click',"#bookingStepb1,#bookingStepb2,#bookingStepb3", function(eve) {
        ...
        else if(vStepId=='bookingStepb3')
        {
            ...
            var token = $("input[name=_token]").val();
            console.log(getPricesSelected());
            sendData = {
                _token:token,
                offer:activeOffer,
                prices:getPricesSelected(),
                additionals:getaditionalSelected(),
    ...
    $(document).on('click', "#bookingStepRet1,#bookingStepRet2,#bookingStepRet3", function (eve) {
        ...
        else if (vStepId == 'bookingStepRet2') {window.history.back();} 
        else if (vStepId == 'bookingStepRet3'){window.history.back();}
    });
    $(document).on('click', "#voucherOffersRet1,#voucherOffersRet2,#voucherOffersRet3", function (eve) {
        ...
        {
            /*window.history.back()*/} else if (vStepId == 'voucherOffersRet3')
        {
            /*window.history.back()*/}
    }
    );
    $(document).on('click', "#voucherOffers2,#voucherOffers3,#voucherOffers3Pay,#voucherOffers4", function (eve) {
        var vStepId = $(eve.target).attr('id');
        ...
        } else if (vStepId == "voucherOffers4")
        {
            ...
            var uCountry = $('#user_country').val();
            var uTelephone = $('#user_telephone').val();
            var bookingDate = $('#user_validFrom').val();
            var uEmail = $('#user_email').val();
            var uMessage = $('#user_message').val();
            var uPayment = $('#user_payment').val();
            var uShipping = $('#user_shipping').val();
            ...
            var vPrice = $('#otherValue').val();
            var optionsValues = $('input[type=radio][name=optionsRadios]:checked');
            var vaucherTempl = $('#voucherType').val();
            if (vaucherCustom == "other")
            {
                dbPayVal = vPrice
            } else {
            ...
            submitOfferBooking(sendData,null);
//            submitOfferBooking(sendData,push_url)
        }
    }
    );
    ...
    $(window).on("popstate", function (e)
    {
        ...
        if (e.state !== null)
        {
            ...
            if (!$(o_a_wh).length)
            {
                ...
                var get_res = navigateMaster(g_url, {
                    urlCorrect: 1, scrollBack: 1, backPressed: 1}
                )
            }
        }
    }
...
window['validateVoucherOffer'] = function (stIn, stNow, fId, fData) {
    ...
    if (stIn == 1)
    {
        ...
        if (stNow == 1)
        {
            cRules = {selectedPricesInp: "required", };
            cMessages = { selectedPricesInp: {required: "Bitte um Ihre Angaben: Ihre Reservationen."}}
        } else if (stNow == 2)
        {
            ...
            if (creditCardR == 1)
            {
                ...
                cRules = {
                    user_name: {
                        required: !0, normalizer: function (value) {
                            return $.trim(value)
                        }
                    }
                    , user_surname: {
                    ...
                    , user_payment: "required", user_shipping: "required", termsCheck: "required"}
                ;
                ...
                cRules = {
                    user_name: "required", user_surname: "required", user_address: "required", user_postalCode: "required", user_postalPlace: "required", user_country: "required", user_telephone: "required", user_email: "required", user_payment: "required", user_shipping: "required", termsCheck: "required", }
                ;
                cMessages = {
                    user_name: {
                        required: "Bitte um Ihre Angaben: Vorname"}
        ...
        $(fId).validate({
            errorClass: "bad-message", rules: cRules, messages: cMessages, submitHandler: function (form) {
                ...
                if (stNow == 1)
                {
                    ...
                    selectPrices_and_go(token,push_url);
//                    window.location.href = push_url
                } else if (stNow == 2)
                {
                    submitBooking(fData)
    ...
    } else if (stIn == 2)
    {
        ...
        $(fId).validate({
            errorClass: "bad-message", rules: cRules, messages: cMessages, submitHandler: function (form) {
                if (stNow == 1)
                {
                    $("#exampleTextarea").html($("#exampleTextarea").val());
                    var mvfs1 = $('#mainVaucherFormS1').html();
                    var mvfs2 = $('#voucherMainTableTop').html();
                    var mvfs3 = $('#vaucherLastStepTop').html();
                    var pData = {
                        step: 2, imgForm: mvfs1, st2Price: mvfs2, st3Price: mvfs3}
                    ;
                    submitVaucherStFirst(pData)
                } else if (stNow == 2)
                {
                    var mvfs1 = $('#mainVauchContact').html();
                    $('.voucherSteps').removeClass('stepShowVoucher');
...
//            window.history.pushState(null,null,push_url);
                    ...
                    fData.step = 2;
                    fData.imgForm = mvfs1;
                    var mvfs2 = $('#voucherStep3Confirm').html();
                    fData.vaucherConfirmTable = mvfs2;
        ...
        $(fId).validate({
            errorClass: "bad-message", rules: cRules, messages: cMessages, submitHandler: function (form) {
                ...
                if (stNow == 1)
                {
                    ...
                    var getpS = window.location.pathname;
                    var parts = getpS.split('/');
                    var cPartsCount = parts.length - 1;
                    var offerVaucherLinkOffer = parts[cPartsCount];
                    if (offerVaucherLinkOffer)
                    {
                        console.log('window.location.pathname interestingPart', offerVaucherLinkOffer)
                    } else {
                        console.log('parts', parts);
                        offerVaucherLinkOffer = parts[(cPartsCount - 1)];
                        console.log('window.location.pathname interestingPart else ', offerVaucherLinkOffer)
                    }
                ...
                } else if (stNow == 2)
                {
                    ...
                    var confDataOffV = $('#vaucherConfirmationInfo').html();
                    var pData = {
                        step: 3, imgForm: mvfs1, saveConfOffVau: confDataOffV}
                    ;
                    ...
                    if (pre_url.indexOf('step=2') !== -1)
                    {
                     ...
                     submitOfferVaucherTable(pData,push_url);
...
//            window.history.pushState(null,null,push_url)
                } else if (stNow == 3)
                {
                    ...
                    fData.step = 3;
                    fData.imgForm = mvfs1;
                    fData.saveConfOffVau = confDataOffV;
                    submitOfferVaucherTablePay(fData)
                }
...
window['submitGroupOffer'] = function (submitData) {
    $.ajax({
        method: "POST", url: br + "submitGroupApplication", data: submitData}
    ).done(function (msg) {
...
window['submitVaucherStFirst'] = function (submitData) {
    ...
    $.ajax({
        method: "POST", headers: {
            'X-CSRF-TOKEN': crf}
        , url: br + "submitVaucherStepFirst", data: submitData}
    ).done(function (msg) {
        if (msg.trim() == "true")
        {
            $('.loadingStepStyle').removeClass('forcedFlexNone');
            navigateMaster('geschenkgutschein?navigate=1&step=2', {
                manualPushState: "geschenkgutschein?step=2", urlCorrect:true}
            );
            $('.voucherSteps').removeClass('stepShowVoucher');
            $('#voucherStep2').addClass('stepShowVoucher')
        } else {
            console.log('message falseee')
        }
    }
    )
}

window['submitVaucherStSecond'] = function (submitData) {
    $('.loadingStepStyle').addClass('forcedFlexNone');
    var crf = $('meta[name="csrf-token"]').attr('content');
    if (!crf)
    {
    ...
    $.ajax({
        method: "POST", headers: {
            'X-CSRF-TOKEN': crf}
        , url: br + "submitVaucherStepSecond", data: submitData}
    ).done(function (msg) {
        $('.loadingStepStyle').removeClass('forcedFlexNone');
        console.log(msg)
    }
...
window['submitVaucherStSecondPay'] = function (submitData) {
    $('.loadingStepStyle').addClass('forcedFlexNone');
    var crf = $('meta[name="csrf-token"]').attr('content');
    if (!crf)
    {
    ...
    $.ajax({
        method: "POST", headers: {
            'X-CSRF-TOKEN': crf}
        , url: br + "submitVaucherStepSecondTest", data: submitData}
    ).done(function (msg) {
        $('.loadingStepStyle').removeClass('forcedFlexNone');
...
window['submitVaucherStThird'] = function (submitData) {
    $('.loadingStepStyle').addClass('forcedFlexNone');
    var crf = $('meta[name="csrf-token"]').attr('content');
    if (!crf)
    {
    ...
    $.ajax({
        method: "POST", headers: {
            'X-CSRF-TOKEN': crf}
        , url: br + "submitVaucherStepThird", data: submitData}
    ).done(function (msg) {
        $('.loadingStepStyle').removeClass('forcedFlexNone');
        console.log(msg)
    }
...
window['submitOfferVaucherTable'] = function (submitData,next=null) {
    $('.loadingStepStyle').addClass('forcedFlexNone');
    $.ajax({
        method: "POST", headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
        , url: br + "submitOfferVaucherTable", data: submitData}
    ).done(function (msg) {
        $('.bookingSteps').removeClass('stepShowBooking');
        $('.bookingSteps').removeClass('stepShowBookingSpecial');
...
window['submitOfferVaucherTablePay'] = function (submitData) {
    $('.loadingStepStyle').addClass('forcedFlexNone');
    $.ajax({
        method: "POST", headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
        , url: br + "submitOfferVaucherTablePay", data: submitData}
    ).done(function (msg) {
        $('#bookingVouchers5').html(msg);
        $('#creditSub').trigger('click');
        $('.loadingStepStyle').removeClass('forcedFlexNone');
        console.log(msg)
    }
...
window['submitContact'] = function (submitData) {
    $.ajax({
        method: "POST", url: br + "submitContact", data: submitData}
    ).done(function (msg) {
...
window['submitVoucher'] = function (submitData) {
    $('.loadingStepStyle').addClass('forcedFlexNone');
    $.ajax({
        method: "POST", url: br + "submitVaucher", data: submitData}
    ).done(function (msg) {
        $('.loadingStepStyle').removeClass('forcedFlexNone');
        $('.voucherSteps').removeClass('stepShowVoucher');
...
window['submitBooking'] = function (submitData) {
    $('.loadingStepStyle').addClass('forcedFlexNone');
    $.ajax({
        method: "POST", url: br + "submitBooking", data: submitData}
    ).done(function (msg) {
        $('.loadingStepStyle').addClass('forcedFlexNone');
        console.log(msg);
        var currFullUrl = window.location.href;
...
window['submitOfferBooking'] = function (submitData,push_url=null) {
    $('.loadingStepStyle').addClass('forcedFlexNone');
    $.ajax({
        method: "POST", url: br + "submitOfferVaucher", data: submitData}
    ).done(function (msg) {
        $('.loadingStepStyle').addClass('forcedFlexNone');
        $('.bookingSteps').removeClass('stepShowBookingSpecial');
        $('.bookingSteps').removeClass('stepShowBooking');
...
window["navigateMaster"] = function (cUrl, opt)
{
    var prepareUrl = cUrl;
    var fullUrl = '';
    if (opt.urlCorrect)
...
//        window.history.pushState(null,null,br+opt.manualPushState)
    ...
    $.ajax({
        url: fullUrl, type: 'GET', headers: {
            'allowNavigate': 1}
        , context: document.body, success: function (r, status, xhr) {
            if (fullUrl == "/kontakt" || fullUrl == "/kontakt/")
            {
        ...
        if (!opt.backPressed)
        {
            ...
            var chechSteps = secRpushUrlFin.indexOf("?");
            if (chechSteps < 0)
            {
                if (secRpushUrlFin == '//' || secRpushUrlFin == "/" || secRpushUrlFin == "https://www.meinweekend.ch")
                {
...
function searchFilter()
{
    ...
    var offerRegion = $("#offerRegion").val();
    var offerDuration = $("#offerDuration").val();
    var keyw = $("#searchUser").val();
    ...
    $('#form_navoverview').on("submit", function (event) {
...
window['scrollToVoucher'] = function () {
...
var f_activateSlider = "activateSlider";
window[f_activateSlider] = function () {
    var options = {
        $FillMode: 2, $AutoPlay: 1, $Loop: 1, $SlideshowOptions: {
            $Class: $JssorSlideshowRunner$, $Transitions: ['']}
        , $ThumbnailNavigatorOptions: {
            $Class: $JssorThumbnailNavigator$, $ChanceToShow: 2, $ArrowKeyNavigation: 1, $ArrowNavigatorOptions: {
                $Class: $JssorArrowNavigator$, $ChanceToShow: 2, $Steps: 1}
        }
    }
    ;
...
window['br2nl'] = function ($input) {
    return preg_replace('/<br\s?\/?>/ius', "\n", str_replace("\n", "", str_replace("\r", "", htmlspecialchars_decode($input))))
}

window['setOfferMain'] = function (s) {
    console.log(s);
    totalBookingPrice = 0;
...
window['selectPrices_and_go'] = function (d,rute) {
    $('.loadingStepStyle').addClass('forcedFlexNone');
    let sTable = $('#bookingTable').html();
    var submitData = {
        _token: d, offer: activeOffer, prices: getPricesSelected(), additionals: getaditionalSelected(), firstTable: sTable}
    ;
...
console.log(submitData);
    $.ajax({
        method: "POST", url: "/selectedPrices", data: submitData, headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
    }
    ).done(function (msg) {
        $('.loadingStepStyle').removeClass('forcedFlexNone');
