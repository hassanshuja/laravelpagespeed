...
window['formatBookingDate'] = function (date) {
...
window['formatVaucherDate'] = function (date) {
    var monthNames = ["Januar", "Februar", "März", "April", "Mai", "Juni", "Juli", "August", "September", "Oktober", "November", "Dezember"];
    var days = ['Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'];
    var day = date.getDate();
...
window['formatEnableDate'] = function (date) {
    var monthNames = ["Januar", "Februar", "März", "April", "Mai", "Juni", "Juli", "August", "September", "Oktober", "November", "Dezember"];
    var days = ['Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'];
...
window['formatEnableDate2'] = function (date) {
    var monthNames = ["Januar", "Februar", "März", "April", "Mai", "Juni", "Juli", "August", "September", "Oktober", "November", "Dezember"];
    var days = ['Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'];
...
window['disableDays'] = function (date) {
...
var f_activateDateCalendar = "activateDateCalendar";
window["activateDateCalendar"] = function () {
...
window['navigateMasterData'] = function (cUrl, opt)
{
    ...
    {
        window.location.href =  br + opt.manualPushState + "?" + recursiveEncoded;
...
var f_setOffer = "setOffer"
window[f_setOffer] = function (s) {
    totalBookingPrice = 0;
...
window['getaditionalSelected'] = function(){return aditionalSelected; };
window['getPricesSelected'] = function(){return pricesSelected; };
window['selectPrice'] = function (ev, pId, aPrice) {
...
window['selectAditional'] = function (ev, pId, aPrice) {
    var curr = Number($('#s_curr').val());
    var exch = Number($('#s_exch').val());
    let currOption = ev.target;
    let singlePriceSelect = {
...
window['addUpBill'] = function (aps, ps, curr, exch) {
    let totalBillA = 0;
    if (aps.length > 0)
    {
...
window['startGroupReservation'] = function (e) {
    navigateMaster('gruppenanfrage/' + activeOffer + '/' + e.target.value, {
    }
    )
}

window['roundUp'] = function (num, precision) {
    precision = Math.pow(10, precision)
    return Math.ceil(num * precision) / precision
}

window['configurePrices'] = function (d) {
}

window['selectPrices'] = function (d) {
...
window['selectPricesOfferVaucher'] = function (d, offVauchUrl) {
    $('.loadingStepStyle').addClass('forcedFlexNone');
    let sTable = $('#bookingTable').html();
//    var sTable = '';
    var submitData = {
    ...
    $.ajax({
        method: "POST", url: "/selectedPrices", data: submitData}
    ).done(function (msg) {
        console.log('msg',msg);
        $('.loadingStepStyle').removeClass('forcedFlexNone');
        if (msg)
        {
            ...
            navigateMaster('offerVaucherReservation/' + offVauchUrl + '?navigate=1&step=2', {
                manualPushState: 'offerVaucherReservation/' + offVauchUrl + '?navigate=1&step=2',
                urlCorrect:true
            }
            );
            if (msg.table)
...
$(document).ready(function () {
 ...
 $(document).on('click', "#bookingStepb1aaa,#bookingStepb2aaa,#bookingStepb3aaa", function (eve) {
