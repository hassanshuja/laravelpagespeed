var br = "http://meinweekend.virtual/";
    ...
    } else if (stIn == 3)
    {
        if (stNow == 1)
        {
            cRules = {
                ...
                , selectedPricesInp: "required", vouchText: {
                    required: !0, normalizer: function (value) {
                        return $.trim(value)
                    }
                }
...
function submitVaucherStFirst(submitData) {
    ...
    $.ajax({
        ...
        if (msg.trim() == "true")
        {
            ...
            navigateMaster('mainVaucherReservation?navigate=1&step=2', {
                manualPushState: "geschenkgutschein?step=1"}
            );
            $('.voucherSteps').removeClass('stepShowVoucher');
...
function submitOfferVaucherTablePay(submitData) {
    $('.loadingStepStyle').addClass('forcedFlexNone');
    $.ajax({
        method: "POST", headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
...
//        window.history.pushState(null,null,br+opt.manualPushState)
    ...
    $.ajax({
        ...
        , context: document.body, success: function (r, status, xhr) {
            if (fullUrl == "https://www.meinweekend.ch/kontakt" || fullUrl == "https://www.meinweekend.ch/kontakt/")
            {
