...
{for(var i=0;i<disDays.length;i++){if($.inArray(myDate,disDays)!=-1){closeDateExists=!0;return!1}
    ...
    {}
        ...
        if(calendarValid==0)
        {}}}
function activateDateCalendar(){console.log('activateDae');stTime=new Date($('#stTime')[0].value*1000),endTime=new Date($('#endTime')[0].value*1000),stFromDate=new Date($('#defDate')[0].value*1000);let cvA=parseInt($('#calendarValid')[0].value);var calendarValid=cvA;$("#datetimepicker12").datepicker("destroy");var today=new Date();var dd=today.getDate();var mm=today.getMonth()+1;var yyyy=today.getFullYear();if(dd<10){dd='0'+dd}
    if(mm<10){mm='0'+mm}
...
{pricesSelected.forEach(function(prices,index){if(prices.priceId==singlePriceSelect.priceId)
{pricesSelected.splice(index,1)}
else{}});pricesSelected.push(singlePriceSelect)}
else{pricesSelected.push(singlePriceSelect)}
    let totalBill=0;pricesSelected.forEach(function(prices,index){totalBill+=Number(prices.totalPrice)*Number(prices.priceValue)});addUpBill(aditionalSelected,pricesSelected,curr,exch)}
...
function selectAditional(ev,pId,aPrice){var curr=Number($('#s_curr').val());var exch=Number($('#s_exch').val());let currOption=ev.target;let singlePriceSelect={additionalId:pId,additionalValue:currOption.value,totalAdditional:aPrice}
    ...
    {aditionalSelected.forEach(function(prices,index){if(prices.additionalId==singlePriceSelect.additionalId)
    {aditionalSelected.splice(index,1)}
    else{}});aditionalSelected.push(singlePriceSelect)}
    else{aditionalSelected.push(singlePriceSelect)}
    let totalBillA=0;aditionalSelected.forEach(function(prices,index){totalBillA+=Number(prices.totalAdditional)*Number(prices.additionalValue)});addUpBill(aditionalSelected,pricesSelected,curr,exch)}
function addUpBill(aps,ps,curr,exch){let totalBillA=0;if(aps.length>0)
{aps.forEach(function(prices,index){totalBillA+=Number(prices.totalAdditional)*Number(prices.additionalValue)})}
    if(ps.length>0)
    {ps.forEach(function(prices,index){totalBillA+=Number(prices.totalPrice)*Number(prices.priceValue)})}
    if(curr==0)
...
{submitData.offerVaucherTable=vauOffE.html()}
    $.ajax({method:"POST",url:br+"selectedPrices",data:submitData}).done(function(msg){console.log('submit');$('.loadingStepStyle').removeClass('forcedFlexNone');if(msg)
