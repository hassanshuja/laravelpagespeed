...
{for(var i=0;i<disDays.length;i++){if($.inArray(myDate,disDays)!=-1){closeDateExists=!0;console.log("disDays days false");return!1}
else{return!0}}}
...
else{console.log("enable days false");return!1}}}
if(openDateExists!==!0)
{if(date>stTime&&date<endTime)
{}
else{console.log("start time false");return!1}
if(calendarValid==0)
{}}}
...
if(mm<10){mm='0'+mm}
today=mm+'/'+dd+'/'+yyyy;console.log('today',today);var stDatee=formatEnableDate2(stFromDate);console.log("comparable",stDatee);if(today==stDatee)
...
else{var replacedReserverd=$('#closedDates')[0].value.replace(/ /g,'');var replaceAllRes=replacedReserverd.replace(/(?:\r\n|\r|\n)/g,'');var closedDates=JSON.parse(replaceAllRes);var avDays=$('#availableDays')[0].value.replace(/ /g,'');var allAvDays=avDays.replace(/(?:\r\n|\r|\n)/g,'');disDays=[];enableDays=[];disDays=closedDates;console.log('disDays',disDays.length);console.log('replacedReserverd',replacedReserverd,'replaceAllRes',replaceAllRes,'closedDates',closedDates,'avDays',avDays,'allAvDays',allAvDays);var cOptions;if(disDays.length>0)
...
if(udch2)
{udch2.html(convDate)}
bookingDate=serverDate;let sendData={selDate:serverDate};let strSD=$.param(sendData);let pData={step:2,pushD:strSD};navigateMasterData('reservation/'+activeOffer,{manualPushState:'reservation/'+activeOffer,type:"GET",data:pData})});console.log('stFromDate',stFromDate);$('#datepicker').datepicker('setStartDate',stFromDate)}}
...
let selecP='#price_'+pId;$(selecP+' option').filter(function(item){console.log("val",currOption.value);console.log("item",item);if(item==currOption.value)
...
let totalBill=0;pricesSelected.forEach((prices,index)=>{totalBill+=Number(prices.totalPrice)*Number(prices.priceValue)});console.log('aditionalSelected',aditionalSelected,'pricesSelected',pricesSelected);addUpBill(aditionalSelected,pricesSelected,curr,exch)}
else{let selecP='#price_'+pId;$(selecP+' option').filter(function(item){console.log("val",currOption.value);console.log("item",item);if(item==currOption.value)
{$(selecP+' option[value='+currOption.value+']').attr('selected','selected')}
...
pricesSelected.forEach((prices,index)=>{if(prices.priceId==singlePriceSelect.priceId)
{pricesSelected.splice(index,1)}
else{}});console.log('aditionalSelected',aditionalSelected,'pricesSelected',pricesSelected);addUpBill(aditionalSelected,pricesSelected,curr,exch)}}
...
console.log(aps.length,ps.length);if(aps.length==0&&ps.length==0)
{console.log("making 0");$("#selectedPricesInp").val('')}
else{console.log("making 1");$("#selectedPricesInp").val('1')}}
...
function selectPrices(d){let sTable=$('#bookingTable').html();var submitData={_token:d,offer:activeOffer,prices:pricesSelected,additionals:aditionalSelected,firstTable:sTable};let vauOffE=$("#vaucherOfferT1");if(vauOffE.length>0)
{submitData.offerVaucherTable=vauOffE.html()}
$.ajax({method:"POST",url:br+"selectedPrices",data:submitData,headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')}}).done(function(msg){if(msg)
{if(msg.table)
